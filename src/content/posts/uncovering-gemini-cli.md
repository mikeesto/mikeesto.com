...
To understand how these tools work, I decided to take a closer look at the newest entrant to the space.
...
Since it's open source, soon after its release I decided to spend a Saturday exploring the codebase. The CLI has been built with Node.js and TypeScript. I was particularly interested in two things: how it determines which files to use as context for the LLM, and how it makes edits to files.
...
This isn't just a simple string passed to the model. Behind the scenes, the CLI's `atCommandProcessor` (from `packages/cli/src/ui/hooks/atCommandProcessor.ts`) intercepts your prompt. It recognizes `@src/main.ts` as a path and invokes the internal `read_many_files` tool (found in `packages/core/src/tools/read-many-files.ts`).
...
For a request like: "In src/main.ts, change the timeout from 5000 to 10000 and add a comment", the model will generate a tool call that looks something like this:
